...
export project_directory='/projects/p30954/test'
...
for sort in `seq 1 $sessions`;do
	if [ -d "$raw_directory/$sort" ]; then
		...
		mv $raw_directory/*MR$sort $raw_directory/$sort
	fi
...
for sess in `seq 1 $sessions`;do
	...
	for sub in */;do
		...
		echo "Transferring!"
		find $raw_directory/$sess/$sub/ -iname *.nii -exec ln -s {} $project_directory/sourcedata/$sub/$sess/ \;
	done
done
