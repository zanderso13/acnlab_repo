...
max_length=50                        # Maximum fiber length in mm
turn_thresh=0
seed_count=2000000
...
singularity exec /home/zaz3744/ACNlab/software/singularity_images/dsi-studio-docker.sif /dsistudio/dsi_studio_64/dsi_studio --action=trk --source=${fibfile} --method=0 --fa_threshold=${fa_threshold} --turn_thresh=${turn_thresh} --initial_dir=0 --seed_plan=1 --interpolation=0 --step_size=${step_size} --smoothing=${smoothing} --min_length=${min_length} --max_length=${max_length}} --seed_count=${seed_count} --fiber_count=${fiber_count} --seed_count=${seed_count} --seed=FreeSurferSeg:Right-Accumbens-area --end=FreeSurferDKT:right_medial_orbitofrontal --export=stat,qa,dti_fa --output=${projdir}/$1-tracks.trk.gz

#delete the trk files so my computer doesn't die
