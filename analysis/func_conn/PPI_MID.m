...
datadir = '/Users/zaz3744/Documents/current_projects/ACNlab/BrainMAPD/func_conn/ICA/MID_data';
maskdir = '/Users/zaz3744/Documents/current_projects/ACNlab/masks/ROI_BrainMAPD_functional/anticipation/left';
timecoursedir = '/Users/zaz3744/Documents/current_projects/ACNlab/BrainMAPD/func_conn/PPI/time_courses/L_HO_Accumbens';
fnames = filenames(fullfile(datadir, '*.nii'));
maskobj = fmri_data(filenames(fullfile(maskdir,strcat('HO_L_Accumbens*.nii'))));

for sub = 1:length(fnames)
    curr_id = fnames{sub}(88:92);
    check_fname = filenames(fullfile(timecoursedir, strcat('L_HO_Accumbens_',curr_id,'_time_series.mat')));
    if isempty(check_fname) == 1
        ...
        curr_save_file = strcat('L_HO_Accumbens_',curr_id,'_time_series.mat');
...
fnames = filenames(fullfile(datadir, '*.nii'));
ppimdldir = '/Users/zaz3744/Documents/current_projects/ACNlab/BrainMAPD/func_conn/PPI/mdl_dir/consumption/LHO_Accumbens_to_wholebrain';
spmdir = '/Users/zaz3744/Documents/current_projects/ACNlab/BrainMAPD/func_conn/first_levels/first_level_output/consumption';

timecoursedir1 = '/Users/zaz3744/Documents/current_projects/ACNlab/BrainMAPD/func_conn/PPI/time_courses/L_HO_Accumbens';
...
% first is where your stats files will be output to
directories{1} = '/Users/zaz3744/Documents/current_projects/ACNlab/BrainMAPD/func_conn/PPI/ppi_fldir/consumption/LHO_Accumbens_to_wholebrain';
% next is where the preprocessed data is
directories{2} = '/Users/zaz3744/Documents/current_projects/ACNlab/BrainMAPD/func_conn/ICA/MID_data';
% the timing files for modelling (onsets, durations, names)
directories{3} = '/Users/zaz3744/Documents/current_projects/ACNlab/BrainMAPD/func_conn/final_timing_files/run-1/consumption/spm_all_vs_0_timing';
% where your extra covariates are including PPI regressors
directories{4} = '/Users/zaz3744/Documents/current_projects/ACNlab/BrainMAPD/func_conn/PPI/mdl_dir/consumption/LHO_Accumbens_to_wholebrain';
...
% specify which mask you're looking at. It should just be the first
% characters of the file
mask_string = 'LHO_Accumbens'; % OFC, VS, HO_VMPFC
...
for sub = 1:length(fnames)
    ...
    if isempty(spm_fname) == 0
        ...
        run_subject_firstlevel_PPI_MID_consumption(PID, ses, run, mask_string, directories, overwrite)
    end
end
...
%% Pull all results from deep storage
...
curr_analysis_dir = {'ROFC_to_wholebrain','RHO_Accumbens_to_wholebrain','LOFC_to_wholebrain','LOFC2_to_wholebrain','biOFC_to_wholebrain','LHO_Accumbens_to_wholebrain'};
    ...
    if roi == 1
        for sub = 1:length(curr_fnames_gain)
            ...
            if isempty(find(clinical_info.PID(:) == PID(sub,1))) == 0
                ...
                disp(strcat(curr_fnames_gain{sub}(95:99), ' missing clinical info'))
                life_dep(sub,1) = 0;%NaN;
                life_anx(sub,1) = 0;%NaN;
                life_com(sub,1) = 0;%NaN;
            end
...
%% quick visualization loop
analysis_dir = '/Users/zaz3744/Documents/current_projects/ACNlab/BrainMAPD/func_conn/PPI/results/consumption';
analysis_type = 'Robust'; % Also have OLS

curr_analysis_dir = {'ROFC_to_wholebrain','RHO_Accumbens_to_wholebrain','LOFC_to_wholebrain','LOFC2_to_wholebrain','biOFC_to_wholebrain','LHO_Accumbens_to_wholebrain'};

load_fname = filenames(fullfile(analysis_dir,strcat(analysis_type,'*')));
load(load_fname{1})
...
%% Above creates the results structures. 
% This section is going to pull clusters and label them

for roi = 1:length(curr_analysis_dir)
    for_label_gain = results_struct.gain.(curr_analysis_dir{roi});
    for_label_loss = results_struct.loss.(curr_analysis_dir{roi});
    anxiety_cluster_gain = region(for_label_gain);
    anxiety_cluster_loss = region(for_label_loss);
%    for group = 1:2
%         for_label_loss.(curr_analysis_dir{roi}).p(:,1) = [];
%         for_label_loss.(curr_analysis_dir{roi}).ste(:,1) = [];
%         for_label_loss.(curr_analysis_dir{roi}).dat(:,1) = [];
%         for_label_loss.(curr_analysis_dir{roi}).threshold(1) = [];
%         for_label_loss.(curr_analysis_dir{roi}).removed_images(1)=[];
%         if group == 1
%             depression_cluster_loss = region(for_label_loss.(curr_analysis_dir{roi}));
%         elseif group == 2
%             comorbid_cluster_loss = region(for_label_loss.(curr_analysis_dir{roi}));
%         end
%         for_label_gain.(curr_analysis_dir{roi}).p(:,1) = [];
%         for_label_gain.(curr_analysis_dir{roi}).ste(:,1) = [];
%         for_label_gain.(curr_analysis_dir{roi}).dat(:,1) = [];
%         for_label_gain.(curr_analysis_dir{roi}).threshold(1) = [];
%         for_label_gain.(curr_analysis_dir{roi}).removed_images(1)=[];
%         if group == 1
%             depression_cluster_gain = region(for_label_gain.(curr_analysis_dir{roi}));
%         elseif group == 2
%             comorbid_cluster_gain = region(for_label_gain.(curr_analysis_dir{roi}));
%         end
%     end  
keyboard
end
